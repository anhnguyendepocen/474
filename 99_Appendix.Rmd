...
ggplot(data, aes(x=x, y=y)) +
  ...
  scale_y_continuous(name=expression(Gamma(x)), breaks=seq(0,12, by=2))
```
...
If the series isn't all the way to $\infty$, the the following is handy result.
$$\sum_{x=0}^{k-1} \alpha^x = \frac{1-\alpha^k}{1-\alpha}\;\;\;\; \textrm{ if } \;\;\; \alpha \ne 1$$
